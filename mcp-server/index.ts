...
/**
 * Load env vars (notably GEMINI_API_KEY) from the project's .env.local when
 * present, so `npm run mcp` and Claude Desktop launches pick the key up
 * without extra configuration. Real environment variables always win, and
 * values are never logged. Missing file is fine — explanations then fall
